...
union all
    select distinct count(*) as row_count, 2 as expected_count
    from {{ source('test_normalization', 'airbyte_raw_nested_stream_with_complex_columns_resulting_into_long_names') }} -- disable test as nesting is not supported yet (for 'nested_stream_with_complex_columns_resulting_into_long_names') }}
union all
    select distinct count(*) as row_count, 2 as expected_count
    from {{ source('test_normalization', 'airbyte_raw_nested_stream_with_complex_columns_resulting_into_long_names') }} -- disable test as nesting is not supported yet (for 'nested_stream_with_complex_columns_resulting_into_long_names_partition') }}
union all
    select distinct count(*) as row_count, 2 as expected_count
    from {{ source('test_normalization', 'airbyte_raw_nested_stream_with_complex_columns_resulting_into_long_names') }} -- disable test as nesting is not supported yet (for 'disabled_test') }}
-- union all
--    select count(distinct id) as row_count, 3 as expected_count
--    from {{ source('test_normalization', 'airbyte_raw_nested_stream_with_complex_columns_resulting_into_long_names') }} -- disable test as nesting is not supported yet (for 'nested_stream_with_complex_columns_resulting_into_long_names_partition_double_array_data') }}
)
select *
